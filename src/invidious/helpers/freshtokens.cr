module FreshTokens
  ...
  def update_stats
    ...
    if ( !REDIS_CLI.nil? && !REDIS_CLI.empty? )
      ...
      LOGGER.warn("FreshTokens: update_stats: REDIS_CLI = NIL | EMPTY")
  ...
  def get_instance_tokens
    ...
    while ( (po_token == "LOCK" || visitor_data == "LOCK") )
      ...
      LOGGER.debug("FreshTokens: get_instance_tokens: #{CONFIG.freshtokens_instanceid}: user: INST-#{instance_id} waiting on redis key lock")    
    ...
    LOGGER.debug("FreshTokens: get_instance_tokens: #{CONFIG.freshtokens_instanceid}: user: INST-#{instance_id} redis key unlocked")  
    ...
    if ( (po_token != "LOCK" && visitor_data != "LOCK") && ((po_token.nil? || visitor_data.nil?) || (po_token.empty? || visitor_data.empty?)) )
      ...
      LOGGER.debug("FreshTokens: get_instance_tokens: #{CONFIG.freshtokens_instanceid}: needs new tokens")
      ...
      LOGGER.debug("FreshTokens: get_instance_tokens: #{CONFIG.freshtokens_instanceid}: user: INST-#{instance_id} unlocking user key")
      LOGGER.debug("FreshTokens: get_instance_tokens: #{CONFIG.freshtokens_instanceid}: user: INST-#{instance_id} stored instance's tokens")

    else    
      LOGGER.debug("FreshTokens: get_instance_tokens: #{CONFIG.freshtokens_instanceid}: user: INST-#{instance_id} already has tokens")
    end
    
    LOGGER.debug("FreshTokens: get_instance_tokens: #{CONFIG.freshtokens_instanceid}: user: INST-#{instance_id} pot: #{po_token}")
    LOGGER.debug("FreshTokens: get_instance_tokens: #{CONFIG.freshtokens_instanceid}: user: INST-#{instance_id} vdata: #{visitor_data}")
  ...
  def get_user_tokens(useremail : String)
    ...
    while ( (po_token == "LOCK" || visitor_data == "LOCK") )
      ...
      LOGGER.debug("FreshTokens: get_user_tokens: #{CONFIG.freshtokens_instanceid}: user: #{useremail} waiting on redis key lock")    
    ...
    LOGGER.debug("FreshTokens: get_user_tokens: #{CONFIG.freshtokens_instanceid}: user: #{useremail} redis key unlocked")     
    ...
    if ((po_token.nil? || visitor_data.nil?) || (po_token.empty? || visitor_data.empty?))
      ...
      REDIS_DB.set("invidious:#{useremail}:po_token", po_token.strip, CONFIG.freshtokens_user_expiry)
      REDIS_DB.set("invidious:#{useremail}:visitor_data", visitor_data.strip, CONFIG.freshtokens_user_expiry)
      
      LOGGER.debug("FreshTokens: get_user_tokens: #{CONFIG.freshtokens_instanceid}: user: #{useremail} stored user's tokens")
  ...
  def get_anon_tokens
    ...
    while ( (po_token.nil? || visitor_data.nil?) || (po_token.empty? || visitor_data.empty?) )
      ...
      # sleep (removed to speed up response to users)
    ...
    LOGGER.debug("FreshTokens: get_anon_tokens: #{CONFIG.freshtokens_instanceid}: user: #{redis_instanceuserid} pot: #{po_token}")
    LOGGER.debug("FreshTokens: get_anon_tokens: #{CONFIG.freshtokens_instanceid}: user: #{redis_instanceuserid} vdata: #{visitor_data}")
...
#    http_proxy_str = "export http_proxy=#{proxy_str} https_proxy=#{proxy_str} HTTP_PROXY=#{proxy_str} HTTPS_PROXY=#{proxy_str} ; " 
    ...
    LOGGER.debug("FreshTokens: NODE = \"#{@@node}\"")
    
    #LOGGER.debug("FreshTokens: generate_tokens: proxy_str = \"#{proxy_str}\"")
    #LOGGER.debug("FreshTokens: generate_tokens: http_proxy_str = \"#{http_proxy_str}\"")
...
#    http_proxy_str = "export http_proxy=#{proxy_str} https_proxy=#{proxy_str} HTTP_PROXY=#{proxy_str} HTTPS_PROXY=#{proxy_str} ; " 
    ...
    #LOGGER.debug("FreshTokens: generate_tokens_timeout: proxy_str = \"#{proxy_str}\"")
    #LOGGER.debug("FreshTokens: generate_tokens_timeout: http_proxy_str = \"#{http_proxy_str}\"")
